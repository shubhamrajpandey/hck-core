...
export default function ManageResourcesPage() {
  ...
  useEffect(() => {
    ...
    if (role !== "Moderator" || !token) {
      ...
      setIsChecked(true);
      console.log("Moderator token:", token);
    }
  }, [router]);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token =
        localStorage.getItem("token") || sessionStorage.getItem("token");
      const endpoint =
        mode === "extra" ? `/extra-resources/${id}` : `/files/${id}`;
    ...
    <>
      ...
      {isChecked && (
        ...
        >
              ...
              <tbody>
                {loadingResources ? (
                  ...
                  resources.map((res) => (
                    <tr
                      key={res.id}
                      className="border-b border-gray-300 hover:bg-gray-50 transition h-[80px]"
                    >
                      ...
                      <td className="px-6 py-4 flex items-center gap-3">
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(res.id.toString())}
                        >
                          <Trash2 size={18} />
                        </button>
                      </td>
                  ...
                  <tr>
                    ...
                    >
                      {mode === "program"
                        ? "Please select at least a module or week to view program resources"
                        : "No resources found"}
                    </td>
                  </tr>
                )}
          ...
          {/* Pagination (only for extra mode) */}
          {mode === "extra" && (
