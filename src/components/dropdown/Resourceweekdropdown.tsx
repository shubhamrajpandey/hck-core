...
export default function WeekDropdown() {
	...
	useEffect(() => {
		...
		if (selectedModuleContext) fetchData();
	...
	return (
		<div className="relative w-full max-w-md sm:max-w-[200px]">
			<button
				...
				className="group flex items-center justify-between px-4 py-3 bg-white border border-gray-300 rounded-md shadow-sm hover:shadow-md w-full text-[16px] sm:text-[18px]">
				<span className="truncate capitalize">{selectedWeek}</span>
				<svg
					width="18"
					height="18"
					viewBox="0 0 14 8"
				...
				<div className="absolute mt-2 w-full sm:w-[200px] bg-white border border-gray-300 rounded-md shadow-lg z-20 max-h-64 overflow-y-auto">
					{weeks.map((week) => (
						<div
							...
							className={`px-4 py-2 cursor-pointer text-[16px] capitalize sm:text-[18px] truncate hover:bg-gray-100 ${
