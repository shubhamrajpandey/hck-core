...
function ProgramDrop() {
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger
				className="group h-[60px] pl-4 pr-4 rounded border border-gray-300 flex gap-10 justify-between items-center py-3.5 w-full max-w-[300px] text-[17px] tracking-[0.5px] bg-white shadow-sm hover:text-white hover:bg-primary hover:border-none transition duration-200 outline-none"
				disabled={isCategoriesLoading || isCategoriesError}>
				{!isCategoriesLoading && isCategoriesError && (
					<p className="truncate text-center text-red-500">
				...
				{!isCategoriesLoading && !isCategoriesError && (
					<>
						<p className="truncate">{currentCategory || "Categories"}</p>
						<ChevronDown
							size={26}
							className="mt-1 text-gray-500 flex-shrink-0 group-hover:text-white"
						/>
					</>
				)}
			</DropdownMenuTrigger>
			...
			<DropdownMenuContent
				className="min-w-[var(--radix-dropdown-menu-trigger-width)] w-[var(--radix-dropdown-menu-trigger-width)] bg-white border border-gray-300"
				align="start"
				sideOffset={4}>
				{fetchedCategories?.length > 0 &&
					...
					fetchedCategories.map((cat: Category) => (
						<DropdownMenuItem
							key={cat.categoryId}
							className="text-center h-[55px] text-[18px] hover:text-white hover:bg-primary"
							onClick={() => {
				...
				{isCategoriesError && !isCategoriesLoading && (
					<DropdownMenuItem className="h-[55px] text-[18px] text-red-500">
